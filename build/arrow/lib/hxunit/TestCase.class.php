...
class hxunit_TestCase extends hxunit_TestFrameworkPrimitive {
	...
	public function assertNull($value, $pos) {
		$this->assert(hxunit_AType::custom(array(new _hx_lambda(array("pos" => &$pos, "value" => &$value), null, array('v0','v1'), "{
			if(!_hx_equal(\$v1, \$v0)) {
				throw new HException(new hxunit_error_AssertionError(\"value is not null\", _hx_anonymous(array(\"fileName\" => \"TestCase.hx\", \"lineNumber\" => 40, \"className\" => \"hxunit.TestCase\", \"methodName\" => \"assertNull\"))));
			}
		}"), 'execute2')), null, $value, $pos);
	}
