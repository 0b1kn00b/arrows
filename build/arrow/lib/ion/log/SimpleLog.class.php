...
class ion_log_SimpleLog implements ion_log_Log{
	...
	public function __trace($begin, $message) {
		if($this->logger->isActive($this->name)) {
			haxe_Log::trace($begin . $this->parseString($message->shift(), $message), _hx_anonymous(array("fileName" => "SimpleLog.hx", "lineNumber" => 41, "className" => "ion.log.SimpleLog", "methodName" => "__trace")));
		}
	}
	public function parseString($str, $harray) {
		return ion_log_SimpleLog::$pattern->customReplace($str, array(new _hx_lambda(array("harray" => &$harray, "str" => &$str), null, array('val'), "{
			return \$harray->shift();
		}"), 'execute1'));
	}
	public $logger;
	public $name;
	static $pattern;
	function __toString() { return 'ion.log.SimpleLog'; }
}
ion_log_SimpleLog::$pattern = new EReg("\\\$[0-9]", "g");
