...
class arrow_AsyncArrowInstance {
	public function __construct($asynca, $x) { if( !php_Boot::$skip_constructor ) {
		...
		$this->resource = arrow_scheduler_ResourceManager::getResource($this->uuid);
	...
	public function cont($x, $f, $g) {
		...
		$t = $this->queue->pop();
		if($t !== null) {
			$this->resource->notify(call_user_func_array(array(new _hx_lambda(array("f" => &$f, "g" => &$g, "self" => &$self, "t" => &$t, "x" => &$x), null, array('f2','a1','a2'), "{
