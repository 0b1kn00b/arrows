...
class Type {
	...
	static function createInstance($cl, $args) {
		...
		return $inst = $c->getConstructor() ? $c->newInstanceArgs($args->__a) : $c->newInstanceArgs();
	}
	...
	static function getEnumConstructs($e) {
		...
		$r = array(); foreach($sps as $k => $v) $r[] = $k;
		$sps = $rfl->getMethods();
		foreach($sps as $m) { $n = $m->getName(); if($n != '__construct' && $n != '__toString') $r[] = $n; }
		return new _hx_array($r);
	}
