package haxe {
	...
	public class Md5 {
		...
		protected function bitOR(a : int,b : int) : int { if( !flash.Boot.skip_constructor ) {
			var lsb : int = (a & 1) | (b & 1);
		...
		protected function rhex(num : int) : String {
			var str : String = "";
		...
		protected function str2blks(str : String) : Array {
			...
			var i2 : int = 0;
			while(i2 < str.length) {
				blks[i2 >> 2] |= str["charCodeAt"](i2) << (((str.length * 8 + i2) % 4) * 8);
				i2++;
			}
			blks[i2 >> 2] |= 128 << (((str.length * 8 + i2) % 4) * 8);
		...
		protected function doEncode(str : String) : String {
			var x : Array = this.str2blks(str);
		...
		static protected var inst : haxe.Md5 = new haxe.Md5();
	}
}
